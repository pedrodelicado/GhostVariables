...
1-(mean(lm.logprice$residuals^2)/var(log(rhBM.price$price)))
# [1] 0.6982418
(sum.lm.logprice <- summary(lm.logprice))

``` 
```{r}
plot(log(rhBM.price$price[Itr]),lm.logprice$fitted.values,
     main=paste("Corr^2=",round(cor(log(rhBM.price$price[Itr]),lm.logprice$fitted.values)^2,2)))
...
relev.ghost.out <- relev.ghost.var(model=lm.logprice, newdata = rhBM.price[Ite,],
...
plot.relev.ghost.var(relev.ghost.out, n1=n.tr, resid.var=sum.lm.logprice$sigma^2,
